...
function updateSteps() {
    for(let i = 0; i < length; i++)if (i < currentStep) {
        stepList[i].classList.add("active");
        if (i > 0) barInnerList[i - 1].classList.add("active");
    } else {
        stepList[i].classList.remove("active");
        if (i > 0) barInnerList[i - 1].classList.remove("active");
    }
}

//# sourceMappingURL=index.132acae4.js.map
